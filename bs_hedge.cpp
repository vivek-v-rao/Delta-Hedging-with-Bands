	...
	for (int i = 0; i < num_paths; i++) {
		...
		if (num_steps > 0) {
			for (int t = 0; t < num_steps; t++) {
				...
				// Only perform delta hedging if the absolute delta change exceeds the threshold
				if (abs(delta_change) > delta_thresh) {
					...
					prev_delta = current_delta; // Update previous delta only if hedging occurred
				}
